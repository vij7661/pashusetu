...
- **Timestamp:** 2026-08-27T18:08:00+05:30
...
- Added a rejecting digits-only/10-character formatter (mixed/overlength paste is rejected, never normalized), disabled browser autofill hints and suggestions, kept fresh fields empty, added localized invalid-mobile feedback, and convert a validated local number to `+91` only at the API boundary.
...
- Farmer `flutter analyze`: `No issues found! (ran in 9.8s)` on the final run.
- Farmer `flutter test`: `15 passed`.
- Farmer web build with local API override: passed; `Built build\web` in `106.3s` on the final run. Informational Wasm suggestion and existing CupertinoIcons font warning only.
...
- **Malformed-input rejection correction:** `8ede57dcd38ceffa107bddbf482b7f23864a4c47`
