...
Securing funds is also treated as one trust mutation: the payment-intent record, `FUNDS_SECURED` transaction state and audit event share one database commit, the accepted bid must exist, invalid transaction state is rejected with a controlled domain error, and the response is typed. This hardening does not redefine which production actor/provider should ultimately own the secure-funds command.
