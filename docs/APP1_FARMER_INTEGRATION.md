...
Securing funds is also treated as one trust mutation: the payment-intent record, `FUNDS_SECURED` transaction state and audit event share one database commit, with a typed response and controlled failure if the accepted bid is missing. This hardening does not redefine which production actor/provider should ultimately own the secure-funds command.
