...
from app.logistics.models import DeliveryRecord

def locked_weight(db, session_id):
    r=db.scalar(select(WeightReading).where(WeightReading.weighment_session_id==session_id, WeightReading.locked.is_(True)))
    if not r: raise AppError("LOCKED_WEIGHT_NOT_FOUND","Locked weight not found.",404)
...
def evaluate_delivery(db:Session, tx:Transaction, delivery_session:WeighmentSession):
    if not tx.active_agreement_id: raise AppError("AGREEMENT_REQUIRED","Locked agreement required.",409)
    agreement=db.get(Agreement, tx.active_agreement_id)
    origin_session_id=db.scalar(select(__import__("app.marketplace.models",fromlist=["Listing"]).Listing.weighment_session_id).where(__import__("app.marketplace.models",fromlist=["Listing"]).Listing.id==tx.listing_id))
    origin=locked_weight(db, origin_session_id)
    delivery=locked_weight(db, delivery_session.id)
    diff,pct,allowed,ok=calculate_tolerance(origin,delivery,agreement.tolerance_basis_points)
